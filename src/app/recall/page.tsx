...
export default function RecallPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: "alright let's see what actually stuck. what did you learn recently that you want to test yourself on?" }
  ])
  ...
  const send = async () => {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ messages: updated, mode: 'recall' })
    })
      ...
      <div className="w-full max-w-xl flex flex-col h-screen py-8">
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-1">🧠 recall</p>
          </div>
        ...
        <div className="mt-4 flex gap-2 items-center">
          <input
            ...
            placeholder="what did you learn recently?"
            value={input}
