...
export default function NotesPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const res = await fetch(`${B2C_API}/api/b2c/notes/${session.user.id}`)
        if (res.ok) {
  ...
  const filtered = search.trim()
    ? notes.filter(n =>
        n.concept_name.toLowerCase().includes(search.toLowerCase()) ||
        n.full_text.toLowerCase().includes(search.toLowerCase())
      )
    : notes
