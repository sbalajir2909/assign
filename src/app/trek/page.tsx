...
export default function TrekPage() {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: "okay let's go deep. what topic do you want to understand from scratch?" }
  ])
  ...
  const send = async () => {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ messages: updated, mode: 'trek' })
    })
      ...
      <div className="w-full max-w-xl flex flex-col h-screen py-8">
        <div className="mb-6 flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-zinc-500 mt-1">🗺️ trek</p>
          </div>
