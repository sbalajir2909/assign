...
export default function Dashboard() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('assign_roadmaps')
  ...
  const getProgress = (concepts: Concept[]) => Math.round((getMastered(concepts) / concepts.length) * 100)
      ...
      <div style={{ position: 'relative', zIndex: 1, maxWidth: '860px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <div style={{ marginBottom: '32px' }}>
          ...
          <p style={{ fontSize: '14px', color: '#444' }}>
            {roadmaps.length === 0 ? 'no roadmaps yet' : `${roadmaps.length} roadmap${roadmaps.length > 1 ? 's' : ''} · ${roadmaps.reduce((acc, r) => acc + getMastered(r.concepts), 0)} concepts mastered`}
          </p>
        </div>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px' }}>
                    <div>
                      ...
                      <p style={{ fontSize: '12px', color: '#444', fontFamily: "'DM Mono', monospace" }}>
                        {mastered} of {roadmap.concepts.length} concepts mastered · started {new Date(roadmap.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      {progress < 100 && (
                        <button className="continue-btn" onClick={() => router.push('/trek')}>
                          continue →
                        </button>
                      )}
                      <button className="delete-btn" onClick={() => deleteRoadmap(roadmap.id)}>
                        remove
                      </button>
                    </div>
                  </div>
