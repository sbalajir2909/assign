...
export default function ProgressPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const res = await fetch(`${B2C_API}/api/b2c/topics/${uid}`)
        if (res.ok) {
          const data: Topic[] = await res.json()
          setTopics(data)
          // Auto-select from query param or first topic
          const paramTopicId = searchParams.get('topic')
          const first = paramTopicId
            ? data.find(t => t.id === paramTopicId) || data[0]
            : data[0]
          if (first) {
  ...
  const loadProgress = async (uid: string, topicId: string) => {
    try {
      const res = await fetch(`${B2C_API}/api/b2c/progress/${uid}/${topicId}`)
      if (res.ok) {
  ...
  const avgMastery = progress.length > 0
    ? progress.reduce((s, p) => s + p.p_learned, 0) / progress.length
    : 0
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minWidth: 0 }}>
                            ...
                            <span style={{ fontSize: '14px', fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                              KC {p.kc_id.slice(0, 8)}
                            </span>
                            {p.flag_type && <FlagBadge flagType={p.flag_type as FlagType} size="sm" />}
                          </div>
                          <span
                            style={{
                              ...mono,
                              ...
                              flexShrink: 0,
                              color: p.status === 'mastered' ? '#4ade80' : p.status === 'force_advanced' ? '#facc15' : 'var(--muted-foreground)',
                            }}
                          >
                            {p.status}
                          </span>
                        </div>
                        <BktBar value={p.p_learned} />
