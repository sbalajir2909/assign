  ...
  const send = async (messageToSend?: string) => {
    ...
    try {
      const res = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ messages: updated, mode: 'build', code, language })
      })
    ...
    <main className="min-h-screen bg-[#0a0a0a] text-white flex flex-col">
      <div className="flex items-center justify-between px-6 py-4 border-b border-zinc-800">
        ...
        <div className="flex items-center gap-3">
          <select
            ...
            className="bg-zinc-900 text-zinc-300 text-xs rounded-lg px-3 py-2 outline-none border border-zinc-700"
          >
      ...
      <div className="flex flex-1" style={{ height: 'calc(100vh - 65px)' }}>
        <div className="w-1/2 flex flex-col border-r border-zinc-800">
          ...
          <textarea
            className="flex-1 bg-[#1e1e1e] text-zinc-100 text-sm font-mono p-4 outline-none resize-none leading-relaxed"
            value={code}
